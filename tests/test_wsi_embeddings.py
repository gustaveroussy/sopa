...
def test_patches_embeddings_inference_clustering():
    ...
    with pytest.raises(AssertionError):  # two images, and no image_key provided
        sopa.compute_embeddings(sdata, "dummy", 50)

    sopa.compute_embeddings(sdata, "dummy", 50, image_key="he_image")

    assert sdata["dummy_embeddings"].shape == (4, 3)

    sopa.compute_embeddings(sdata, "dummy", 25, level=-1, image_key="he_image")

    assert sdata["dummy_embeddings"].shape == (1, 3)

    sopa.compute_embeddings(sdata, "dummy", 13, patch_overlap=3, level=-1, image_key="he_image")
    ...
    sopa.compute_embeddings(sdata, "dummy", 10, magnification=10, image_key="he_image")
    assert sdata["dummy_embeddings"].shape == (9, 3)

    sopa.compute_embeddings(sdata, "dummy", 11, magnification=10, image_key="he_image")
    assert sdata["dummy_embeddings"].shape == (4, 3)

    sopa.compute_embeddings(sdata, "dummy", 50, magnification=100, image_key="he_image")
    assert sdata["dummy_embeddings"].shape == (16, 3)

    sopa.cluster_embeddings(sdata, "dummy_embeddings", method="kmeans")
...
def test_deterministic_embedding(model: str):
    ...
    sopa.compute_embeddings(sdata_openslide, model, patch_width)
    sopa.compute_embeddings(sdata_on_disk, model, patch_width)
