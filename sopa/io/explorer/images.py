...
class MultiscaleImageWriter:
    ...
    def _should_load_memory(self, shape: tuple[int, int, int], dtype: np.dtype):
        ...
        return size / psutil.virtual_memory().total < self.ram_ratio_threshold
    ...
    def write(self, path, lazy=True, ram_ratio_threshold=None):
        self.lazy = lazy
...
def write_image(
    ...
    image_writer.write(path, lazy=lazy, ram_ratio_threshold=ram_ratio_threshold)
