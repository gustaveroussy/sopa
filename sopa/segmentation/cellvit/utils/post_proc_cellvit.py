...
class DetectionCellPostProcessor:
    def __init__(
        self, nr_types: int = None, magnification: Literal[20, 40] = 40, gt: bool = False,
    ) -> None:
    ...
    def post_process_cell_segmentation(self, pred_map: np.ndarray,) -> Tuple[np.ndarray, dict]:
        ...
        pred_inst = self.__proc_np_hv(pred_inst, object_size=self.object_size, ksize=self.k_size)
...
def calculate_instances(pred_types: torch.Tensor, pred_insts: torch.Tensor) -> list[dict]:
    """Best used for GT
