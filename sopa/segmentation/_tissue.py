...
def tissue(
    ...
    !!! info
        ...
        As a preprocessing step, a median blurring is applied with an element of size `blur_kernel_size`
    ...
    geo_df = TissueSegmentation(
        image, blur_kernel_size, open_kernel_size, close_kernel_size, drop_threshold
    ).get_polygons(mode)
...
class TissueSegmentation:
    ...
    def hsv_otsu(self) -> gpd.GeoDataFrame:
        ...
        thumbnail_hsv_blurred = cv2.medianBlur(thumbnail_hsv[:, :, 1], self.blur_kernel_size)
