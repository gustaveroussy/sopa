...
!!! warning
    Here we only detail the multi-step segmentation. For the rest of the CLI usage, refer to our [CLI usage tutorial](../cli_usage), and only replace the "Run segmentation" section by the instructions below.
...
Now, we can run Cellpose on each of the four patches, and for each "segmentation step" we want. On this toy example, we run 3 steps, with (i) DAPI + CK, (ii) DAPI + CD3, and (iii) DAPI + CD20.
...
At this stage, you executed 12 times Cellpose (3 steps on each of the 4 patches). Now, we need to resolve the conflict, i.e. merging the three segmentations into one. Note that we gave the paths to the temporary boundaries that we made above.
```sh
...
Congrats, you have now merged the results of a three-step segmentation! For all the following task, you can refer to our normal [CLI usage tutorial](../cli_usage).
...
This function should be wrapped into a "method builder". The purpose is that you can provide arguments to the method builder, that can then be used by your desired function. For instance, this is a dummy method builder, whose segmentation function only creates one cell:
...
To use the CLI, you'll need to clone the repository. Also, we advise to install sopa in dev mode, it will allow you to update your segmentation function without the need to re-install everything. For instance: 
...
Congrats, you have now run your own segmentation method! For all the following task, you can refer to our normal [CLI usage tutorial](../cli_usage).
